...
async function deployRandomNumberDirectFundingConsumer(chainId) {
    ...
    if (chainId == 31337) {
        console.log(
            `Run the following command to fund your contract with link tokens: pnpm hardhat transfer-link --recipient ${randomNumberConsumerV2.address} --linkaddress ${linkAddress} --amount REPLACE_AMOUNT_IN_JUELS --network ${network.name}`
        )
    } else {
        console.log(
            `Run the following command to fund your contract with link tokens: pnpm hardhat transfer-link --recipient ${randomNumberConsumerV2.address} --amount REPLACE_AMOUNT_IN_JUELS --network ${network.name}`
        )
    }
}
